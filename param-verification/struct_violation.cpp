    ...
    const cl_image_desc * const image_desc)
{
    ...
    if ((image_desc->image_type == CL_MEM_OBJECT_IMAGE2D) &&
        (image_desc->mem_object != NULL) &&
        is_valid_mem_obj(image_desc->mem_object, "image"))
    {
...
// check image sizes to fit into any device of the context
bool struct_violation(
    const cl_image_desc * const image_desc, 
    cl_context context)
{
    ...
    const cl_image_format * image_format)
{
    ...
    if (image_desc->image_row_pitch % pixel_size(image_format) != 0)
        return true;
    size_t image_row_pitch = std::max(image_desc->image_row_pitch, image_desc->image_width * pixel_size(image_format));
    ...
    switch (image_desc->image_type) {
        case CL_MEM_OBJECT_IMAGE1D_BUFFER:
            if (image_desc->mem_object != NULL)
                if (!is_valid_mem_obj(image_desc->mem_object, "buffer") ||
